    ...
    >
      ...
      <div class="row no-wrap justify-center col-12">
        <div class="column items-center q-mx-sm" v-for="(trial, index) in question.all" :key=index>
          <q-btn :disabled="(state.willPlay[objKey]||0)!==index" @click="change(objKey,index)">
            <div class="row no-wrap">
              <div class="col-6" :style="{position: 'relative', transition: 'all 1s', overflow: 'hidden', maxWidth: (state.willPlay[objKey]||0)===index?'3em':'0em'}">
                <q-spinner :style="{transition: 'all 0.2s', opacity:state.playing?1:0}"/>
                <q-icon style="position: absolute; top:0px; left:0px; }" name="play_arrow" :style="{transition: 'all 0.2s', opacity:state.playing?0:1}"/>
              </div>
              <div class="col-6">
      ...
      <q-slide-transition>
        <div v-show="(state.willPlay[objKey]||0)>=question.all.length" class="row justify-center">
          <q-slide-transition>
  ...
  setup() {
    ...
    const playBoth = (idR,idL,questionName) => {
      ...
      if (!state.hasPlayedDichotic[state.currentQuestion]){//on vérifie que le son n'a pas déjà été joué
        playRight({ id: "English_S01_M01_1" }) //id: idR
        playLeft({ id: "English_S01_M01_1" }) //id:idL
